...
export default function TechnicianChat() {
  const { technician, userType } = useAuth();
  ...
  }, [technician, userType, router, requestId]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim() || !sharedKey || !technician) return;
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-card border-b border-border">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-foreground">Customer Chat (End-to-End Encrypted)</h1>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <Card className="flex flex-col h-96">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 ? (
              <p className="text-center text-muted-foreground">No messages yet. Wait for customer to start.</p>
            ) : (
              messages.map((msg) => (
                <div
                  key={msg.id}
